...
# Save state helper — uses --argjson so band values are stored as JSON numbers
save_preset() {
    jq -n -c --argjson b1 "$1" --argjson b2 "$2" --argjson b3 "$3" --argjson b4 "$4" --argjson b5 "$5" \
          --argjson b6 "$6" --argjson b7 "$7" --argjson b8 "$8" --argjson b9 "$9" --argjson b10 "${10}" --arg p "${11}" \
...
case $cmd in
    ...
    "set_band")
        ...
        # Uses --argjson so the value is stored as a JSON number, not a string.
        tmp=$(cat "$STATE_FILE")
        updated=$(echo "$tmp" | jq -c --argjson val "$arg2" ".b$arg1 = \$val | .preset = \"Custom\" | .pending = true")
